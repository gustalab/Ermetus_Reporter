...
```{r echo=FALSE, include=FALSE}
  ...
  gelir_tablosu <- 
    ...
    tibble::tribble(
      ...
      "1/01/2023-30/06/2023",  8500L,
      "1/07/2023-31/12/2023",  11304L,
...
```{r echo=FALSE, include=FALSE}
  ...
  son_hesaplama_tablosu <- data.frame(Tazminat = c("Bilinen Dönem Tazminatı", "Bilinmeyen Dönem Tazminatı", "Toplam Tazminat", 
                                      ...
                                      Tutar = c(HT_Bilinen_Donem_Tazminatı, HT_Bilinmeyen_Donem_Tazminatı, Toplam_Tazminat, 
                                               Toplam_Bakici_Gideri)
  )
  
  knitr::kable(son_hesaplama_tablosu , caption = "Toplam Tazminat Tablosu")
...
Sayın Hakemliğinizce tarafımıza iletilen dosya kapsamında geçici iş göremezlik, sürekli maluliyet tazminatı ve bakıcı gideri tazminatı hesaplaması Bilirkişiliğimiz teknik bilgisi kapsamında yapılmıştır.
...
knitr::kable(HT_Tam_Maluliyet_Donemi_Tablosu, caption = " Geçici İş Göremezlik Tazminat Tablosu")

knitr::kable(HT_Maluliyet_Sonrasi_Donem_Tablosu, caption = " Sürekli Maluliyet Tazminat Tablosu")
...
str_glue("Bilinen Dönem Sürekli Maluliyet Tazminatı  {hesap_tarihi_bilinen_donem_toplam_tazminat} TL olarak hesaplanmıştır.")
...
Bilinen dönem tazminatı ile bilirkişi hesap tarihinden itibaren bilinmeyen dönem tazminat tutarları ve toplam maluliyet tazminatı turarı aşağıdaki tabloda verilmektedir.

```{r echo=FALSE}

knitr::kable(hesap_tarihi_toplam_tazminat_tablosu, caption = "Toplam Sürekli Maluliyet Tazminat Tablosu")
...
Yerleşik Yargıtay kararları kapsamında bakıcı gideri tazminatı hesaplaması ilgili dönem brüt asgari ücret üzerinden hesaplanmıştır. Hesaplama detayları aşağıda tabloda verilmektedir.

```{r echo=FALSE}

if (Bakıcı_gideri == "Var") {
    knitr::kable(Bakici_Gideri_Donemi_kazanc_hesaplama_tablosu , caption = "Bakıcı Gideri Tazminatı Tablosu") } else {str_glue("Dosya kapsamında bakıcı gideri bulunmamaktadır.")}
  
Toplam_Bakici_gideri_hesaplama = if (Maluliyet_Oranı <= 100) {sum(Bakici_Gideri_Donemi_kazanc_hesaplama_tablosu$Donem_Bakıcı_Gideri) 
...
if (Bakıcı_gideri == "Var") {
    str_glue("Toplam Bakıcı Gideri {Toplam_Bakici_Gideri} TL olarak hesaplanmıştır.") } else {str_glue(" ")} 
...
str_glue(" Bilirkişiliğimize iletilen dosya kapsamında yapılan inceleme neticesinde elde edilen hesaplama parametreleri kapsamında gerçekleşen kazada malul kalan {Ad_Soyad} için davalı şirketin kusur oranı kapsamında kaza tarihli trafik sigortası poliçe teminat limiti kapsamında hesaplanan geçici iş göremezlik, sürekli maluliyet ve bakıcı gideri tazminat tutarları bilirkişi hesap tarihi itibariyle aşağıda belirtilmiştir;")
...
knitr::kable(HT_Tam_Maluliyet_Donemi_Tablosu, caption = " Geçici İş Göremezlik Tazminat Tablosu")
str_glue("* Geçici iş göremezlik tazminatı {toplam_gecici_maluliyet_tazminatı} TL olarak hesaplanmıştır.")

knitr::kable(hesap_tarihi_toplam_tazminat_tablosu, caption = "Toplam Sürekli Maluliyet Tazminat Tablosu")

str_glue("* Sürekli maluliyet tazminatı {Toplam_Tazminat} TL olarak hesaplanmıştır.")

knitr::kable(Bakici_Gideri_Donemi_kazanc_hesaplama_tablosu , caption = "Bakıcı Gideri Tazminatı: ")

str_glue("* Bakıcı Gideri {Toplam_Bakici_Gideri} TL olarak hesaplanmıştır.")
```
