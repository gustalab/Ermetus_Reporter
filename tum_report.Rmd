...
```{r echo=FALSE, include=FALSE}
  ...
  Kısmi_Odeme_Tutarı_2 <- params$PKısmi_Odeme_Tutarı_2
  Yasam_Tablosu <- params$PYasam_Tablosu
  Bakici_Suresi <- params$PBakici_Sure


  # Bakıcı Gideri ----
  
  Bakıcı_gideri <- "Var"
  Bakıcı_gideri_suresi <- Bakici_Suresi
  ...
  gelir_tablosu <- 
    ...
    tibble::tribble(
      ...
      "1/01/2017-31/12/2017",  3115.23L,
      "1/01/2018-31/12/2018",  2967.75L,
      "1/01/2019-31/12/2019",  3660.75L,
      "1/01/2020-31/12/2020",  4211.09L,
      "1/01/2021-31/12/2021",  4636.75L,
      "1/01/2022-30/06/2022",  5000L,
      "1/07/2022-31/12/2022",  5000L,
      "1/01/2023-31/12/2023",  8681.45L
  ...
  Gelir_tablo <-  Asgari_Tablo %>%
    ...
    mutate(Donem_Baslangic = as.Date(dmy(Donem_Baslangic), "%d/%m/%Y"),
           Donem_Son = as.Date(dmy(Donem_Son), "%d/%m/%Y")) %>% 
    mutate(D_B = Donem_Baslangic,
           D_S = Donem_Son)
  ...
  Teminat_Limit_Tablosu_rvz <- Teminat_Limit_Tablosu %>% 
    separate(Donem, sep = "-", into = c("Donem_Baslangic", "Donem_Son")) %>%
    mutate(Donem_Baslangic = as.Date(dmy(Donem_Baslangic), "%d/%m/%Y"),
           Donem_Son = as.Date(dmy(Donem_Son), "%d/%m/%Y")) %>% 
    mutate(D_B = Donem_Baslangic,
  ...
  kaza_tarihi_teminat <- Teminat_Limit_Tablosu_rvz %>% 
...
```{r echo=FALSE, include=FALSE}
  ...
  parametre_tablosu2 <- tibble(
    ...
    "Ödeme Tarihi-1" = as.character(Kısmi_Odeme_Tarihi_1),
    "Ödeme Tutarı-1" = Kısmi_Odeme_Tutarı_1,
    "Ödeme Tarihi-2" = as.character(Kısmi_Odeme_Tarihi_2),
...
# 4- ŞİRKET ÖDEME TARİHİ İLE HESAPLAMA ----
  ...
  Maluliyet_Donemi_tablosu$Donem_Baslangic[1] <- maluliyet_tarihi_baslangıc
  Maluliyet_Donemi_tablosu$Donem_Son[nrow(Maluliyet_Donemi_tablosu)] <- maluliyet_tarihi_bitis
  
  Maluliyet_Donemi_tablosu
  
  Maluliyet_Donemi_tazminat_hesaplama <- Maluliyet_Donemi_tablosu %>% 
    mutate(Donem_Baslangici = Donem_Baslangic,
           Donem_Sonu = Donem_Son) %>% 
...
```{r echo=FALSE, include=FALSE}
  ...
  Maluliyet_Sonrasi_Donem_tablosu$Donem_Baslangic[1] <-  maluliyet_sonrası_donem_baslangıc
  Maluliyet_Sonrasi_Donem_tablosu$Donem_Son[nrow(Maluliyet_Sonrasi_Donem_tablosu)] <- Hesap_Tarihi
...
```{r echo=FALSE, include=FALSE}
  ...
  HT_Maluliyet_Donemi_tablosu$Donem_Baslangic[1] <- maluliyet_tarihi_baslangıc
  HT_Maluliyet_Donemi_tablosu$Donem_Son[nrow(HT_Maluliyet_Donemi_tablosu)] <- maluliyet_tarihi_bitis
  
  HT_Maluliyet_Donemi_tablosu
  ...
  HT_Maluliyet_Sonrasi_Donem_tablosu$Donem_Baslangic[1] <- maluliyet_sonrası_donem_baslangıc
  HT_Maluliyet_Sonrasi_Donem_tablosu$Donem_Son[nrow(HT_Maluliyet_Sonrasi_Donem_tablosu)] <- Hesap_Tarihi
  ...
  HT_Maluliyet_Sonrasi_Donem_tazminat_hesaplama <- HT_Maluliyet_Sonrasi_Donem_tablosu %>% 
    ...
    mutate(Kazanilan_Ay = case_when(
      Kazanilan_Ay >= 11.96 ~ round(Kazanilan_Ay, digits = 1),
      Kazanilan_Ay >= 6.2 ~  plyr::round_any(Kazanilan_Ay, 0.1, f = ceiling),
...
```{r echo=FALSE, include=FALSE}
  ...
  odeme1_gecen_gun <- as.numeric(Hesap_Tarihi - Kısmi_Odeme_Tarihi_1)
  odeme2_gecen_gun <- as.numeric(Hesap_Tarihi - Kısmi_Odeme_Tarihi_2)
  
  
  odeme_hesaplama_degeri_1 <- if (Kısmi_Odeme_Tutarı_1 == 0) {0} else {Kısmi_Odeme_Tutarı_1 * (1 + (faiz_oranı/100) / 360 * odeme1_gecen_gun) }
  
  
  odeme_hesaplama_degeri_2 <- if (Kısmi_Odeme_Tutarı_2 == 0) {0} else {Kısmi_Odeme_Tutarı_2 * (1 + (faiz_oranı/100) / 360 * odeme2_gecen_gun) }
...
```{r echo=FALSE, include=FALSE}
 ...
 # 6- BAKICI GİDERİ HESAPLAMASI ----
  ...
  Bakici_Gideri_Bitis_Tarihi <- Kaza_Tarihi + Bakıcı_gideri_suresi*30
  
  Bakici_Gideri_Donemi_filtered <- Gelir_tablo %>% 
    filter(D_S >= Bakici_Gideri_Baslangic_Tarihi & Bakici_Gideri_Bitis_Tarihi > D_B)
  
  
  Bakici_Gideri_Donemi_filtered$Donem_Baslangic[1] <- Bakici_Gideri_Baslangic_Tarihi
  Bakici_Gideri_Donemi_filtered$Donem_Son[nrow(Bakici_Gideri_Donemi_filtered)] <- Bakici_Gideri_Bitis_Tarihi 
  
  
  Bakici_Gideri_Donemi_kazanc_hesaplama_tablosu <- Bakici_Gideri_Donemi_filtered %>% 
    mutate(Donem_Bas = Bakici_Gideri_Baslangic_Tarihi,
           Donem_Sonu = Bakici_Gideri_Bitis_Tarihi) %>% 
...
Kaza tarihi diğer bir ifadeyle maluliyet tarihi ile şirket ödeme tarihi arasındaki sürede malulün yıllar itibariyle gelir bilgisi, maluliyet oranı, şirket kusur oranı kullanılarak yapılan Bilinen Dönem tazminat tutarları aşağıdaki tabloda yıl bazında detaylı olarak verilmektedir.
...
str_glue("Toplam Geçici Maluliyet Tazminatı {HT_Tam_Maluliyet_Donemi_Tablosu$Donem_Tazminat} TL olarak hesaplanmıştır.")
